...
function Panel5({ gestures = [], style = {}, selectionStyle = {}, accessibilityLabel, accessibilityHint }) {
  ...
  const row = (0, _reactNativeReanimated.useSharedValue)(0);
  const column = (0, _reactNativeReanimated.useSharedValue)(0);
  const adaptiveColor = (0, _reactNativeReanimated.useSharedValue)('#000');
  const width = (0, _reactNativeReanimated.useSharedValue)(0);
  const height = (0, _reactNativeReanimated.useSharedValue)(0);
  const squareSize = (0, _reactNativeReanimated.useDerivedValue)(
    () => Math.max(width.value / 12, height.value / 10),
    (0, _utils.getWebDependencies)([width, height]),
  );
  const setAdaptiveColor = color => {
    'worklet';

    const isDark = _index.default.runOnUI().isDark(color);
  ...
  (0, _reactNativeReanimated.useDerivedValue)(
    ...
    (0, _utils.getWebDependencies)([hueValue, saturationValue, brightnessValue, row, column]),
  );
  const selectedStyle = (0, _reactNativeReanimated.useAnimatedStyle)(
    ...
    (0, _utils.getWebDependencies)([squareSize, adaptiveColor, row, column]),
  );
  const onUpdate = (newColumn, newRow) => {
    'worklet';

    var _GRID_COLORS$newRow;
    const color =
      (_GRID_COLORS$newRow = GRID_COLORS[newRow]) === null || _GRID_COLORS$newRow === void 0
        ? void 0
        : _GRID_COLORS$newRow[newColumn];
    if (!color) return;
