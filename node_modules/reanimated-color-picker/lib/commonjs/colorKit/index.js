...
const colorKitUI = () => {
  ...
  const matchColorRegex = (color, regexArray) => {
    for (const regex of regexArray) {
      const matches = color.match(regex);
      if (matches) {
        return [parseInt(matches[1], 10), parseFloat(matches[2]), parseFloat(matches[3]), parseFloat(matches[4] ?? '1')];
      }
    }
  };

  /** Only for hex (base 16) */
  const matchHexRegex = (color, regex) => {
    if (!regex.test(color)) return;
    const hex8 = normalize_HEX(color);
    const matches = hex8.match(COLORS_REGEX.hex8);
    if (!matches) return;
    return [parseInt(matches[1], 16), parseInt(matches[2], 16), parseInt(matches[3], 16), parseInt(matches[4], 16) / 255];
  };

  /** Check if a color object has all the required keys */
  const isColorObject = (colorObject, keys) => {
    return keys.every(k => colorObject.hasOwnProperty(k) && typeof colorObject[k] === 'number');
  };
  const clampColorObject = color => {
    const castColor = color;
  ...
  const forceColorObjectAlpha = color => {
  ...
  const RGB_to_HEX = ({ r, g, b, a }, forceAlpha) => {
  ...
  const HSL_to_HEX = (color, forceAlpha) => {
  ...
  const HWB_to_HEX = (color, forceAlpha) => {
  ...
  const spin = (color, degree) => {
    ...
    const newColor = {
      h: Math.round((((h + spinDegree) % 360) + 360) % 360),
      s,
      l,
      a,
    };
  ...
  const parse = color => {
    ...
    if (typeof color === 'string') {
      ...
      if (NAMED_COLORS.hasOwnProperty(color)) {
        const matches = matchHexRegex(NAMED_COLORS[color], COLORS_REGEX.hex6);
        if (matches) {
          return {
            format: 'named',
            value: {
